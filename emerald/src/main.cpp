...
DWORD shellCode(ManualMappingData *data)
{
    ...
    auto *optionalHeader =
        &reinterpret_cast<PIMAGE_NT_HEADERS>(base + reinterpret_cast<PIMAGE_DOS_HEADER>(data)->e_lfanew)
             ->OptionalHeader;
    ...
    if (locationDelta != 0)
    {
        ...
        auto *relocData = reinterpret_cast<PIMAGE_BASE_RELOCATION>(
        ...
        while (relocData->VirtualAddress != 0U)
        {
            ...
            relocData =
                reinterpret_cast<PIMAGE_BASE_RELOCATION>(reinterpret_cast<PBYTE>(relocData) + relocData->SizeOfBlock);
        }
    }
    ...
    if (optionalHeader->DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size != 0)
    {
        ...
        while (importDesc->Characteristics != 0)
        {
            ...
            while (originalFirstThunk->u1.AddressOfData != 0U)
            {
                ...
                {
                    auto loadedFunction =
                        getProcAddress(hDll, reinterpret_cast<LPCSTR>(originalFirstThunk->u1.Ordinal & 0xFFFF));
...
bool memory_map(HANDLE proc, const fs::path &path)
{
    ...
    {
        spdlog::error(L"Invalid payload platform. Expected "
                      L"IMAGE_FILE_MACHINE_AMD64({:X}) but got {:X}.",
                      IMAGE_FILE_MACHINE_AMD64, oldFileHeader->Machine);
        return false;
    }

    spdlog::trace(L"Attempting to allocate memory at the preferred base address "
                  L"for payload");
    auto *targetBase = reinterpret_cast<uint8_t *>(
        VirtualAllocEx(proc, reinterpret_cast<void *>(oldOptHeader->ImageBase), oldOptHeader->SizeOfImage,
                       MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
    if (targetBase == nullptr)
    {
        spdlog::trace("-- Preferred address unavailable, trying for any address");
        targetBase = reinterpret_cast<uint8_t *>(
            VirtualAllocEx(proc, nullptr, oldOptHeader->SizeOfImage, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
